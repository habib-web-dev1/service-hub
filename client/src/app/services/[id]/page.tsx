...
import { ArrowLeft, Star, Clock, Tag, Mail, Phone, Calendar, User, MessageSquare, CheckCircle, Shield } from "lucide-react";
...
const categoryImages: Record<string, string> = {
  plumbing: "https://images.unsplash.com/photo-1584622650111-993a426fbf0a?w=1000&auto=format&fit=crop",
  cleaning: "https://images.unsplash.com/photo-1581578731548-c64695cc6952?w=1000&auto=format&fit=crop",
  electrical: "https://images.unsplash.com/photo-1621905251189-08b45d6a269e?w=1000&auto=format&fit=crop",
  photography: "https://images.unsplash.com/photo-1516035069371-29a1b244cc32?w=1000&auto=format&fit=crop",
  fallback: "https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?w=1000&auto=format&fit=crop"
};
    ...
    return (
      <main className="min-h-screen bg-slate-950 px-6 py-12 text-white">
        <div className="mx-auto max-w-3xl">
          <Link href="/services" className="flex items-center gap-1 text-sm text-cyan-400 hover:underline">
            <ArrowLeft className="h-4 w-4" /> Back to listings
          </Link>
          <div className="mt-8 rounded-xl border border-red-500/30 bg-red-500/10 p-8 text-center">
            <h1 className="text-2xl font-bold text-red-400">Service Not Found</h1>
            <p className="mt-3 text-red-300">{error || "The service details could not be retrieved."}</p>
          </div>
        </div>
      </main>
    );
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="rounded-2xl border border-slate-850 bg-slate-900/40 p-6 md:p-8 backdrop-blur-md">
              ...
              <div className="mt-4 flex flex-wrap gap-4 text-sm text-slate-400 items-center">
                <div className="flex items-center gap-1 text-yellow-400 font-semibold">
                  ...
                  {reviews.length > 0 && <span className="text-slate-500 font-normal">({reviews.length} reviews)</span>}
                </div>
                <span>•</span>
              ...
              <div className="mt-8 border-t border-slate-800/80 pt-6">
                <h3 className="text-lg font-bold text-slate-200 mb-3">Service Description</h3>
              ...
              {/* Features List */}
              <div className="mt-8 border-t border-slate-800/80 pt-6">
                <h3 className="text-lg font-bold text-slate-200 mb-3 font-semibold">What is included</h3>
                <ul className="grid gap-2.5 sm:grid-cols-2 text-xs md:text-sm text-slate-400">
                  <li className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-green-400 shrink-0" /> Vetted local professional</li>
                  <li className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-green-400 shrink-0" /> Service tools & equipment</li>
                  <li className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-green-400 shrink-0" /> Appointment insurance</li>
                  <li className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-green-400 shrink-0" /> 100% moneyback guarantee</li>
                </ul>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-slate-850 bg-slate-900/40 p-6 md:p-8 backdrop-blur-md space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800/60 pb-5">
                <div>
                  <h3 className="text-xl font-bold text-slate-200">Customer Feedbacks</h3>
                  <p className="text-xs text-slate-500 mt-1">Direct feedback from verified bookings</p>
                </div>
                {reviews.length > 0 && (
                  <div className="flex items-center gap-3 rounded-xl bg-slate-950 px-4 py-2 border border-slate-850">
                    ...
                    <div>
                      <div className="flex text-yellow-400">
                        {[...Array(5)].map((_, i) => (
                          <Star key={i} className={`h-3 w-3 ${i < Math.round(averageRating) ? "fill-yellow-400" : "text-slate-800"}`} />
                        ))}
                      </div>
              ...
              {reviewsError && <div className="text-sm text-red-400">{reviewsError}</div>}
              {reviewsLoading ? (
                ...
                <div className="text-center py-10 text-slate-500">
                  ...
                  <p className="text-sm">No reviews left for this service yet.</p>
                  <p className="text-xs text-slate-600 mt-1">Book the service and be the first to share feedback!</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {reviews.map((rev) => (
                    <div key={rev.id} className="rounded-xl bg-slate-950/40 p-4 border border-slate-900">
                      <div className="flex justify-between items-start gap-4">
                        <div>
                          <p className="font-bold text-slate-300 text-xs md:text-sm">{rev.user?.name || "Client"}</p>
                          <div className="flex text-yellow-400 gap-0.5 mt-1">
                            {[...Array(5)].map((_, i) => (
                              <Star key={i} className={`h-3.5 w-3.5 ${i < rev.rating ? "fill-yellow-400 text-yellow-400" : "text-slate-800"}`} />
                            ))}
                          </div>
                        </div>
                        <span className="text-[10px] text-slate-600">{new Date(rev.createdAt).toLocaleDateString()}</span>
                      </div>
                      {rev.comment && (
          ...
          <div className="lg:col-span-1 space-y-6 lg:sticky lg:top-24">
            ...
            <div className="rounded-2xl border border-cyan-500/20 bg-slate-900/60 p-6 backdrop-blur-md shadow-xl relative overflow-hidden">
              ...
              <div className="mb-6">
                ...
                <p className="text-3xl font-black text-cyan-400 mt-1">
                  ...
                  <span className="text-xs text-slate-400 font-normal"> / job</span>
                </p>
              </div>
              ...
              <div className="space-y-4 border-t border-slate-800 pt-5">
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="font-semibold">{service.duration ? `${service.duration} minutes` : "Flexible"}</span>
                </div>
              ...
              <button
                ...
                className="mt-8 w-full rounded-xl bg-gradient-to-r from-cyan-500 to-blue-500 py-3.5 text-sm font-bold text-slate-950 transition hover:opacity-95 shadow-md shadow-cyan-500/5 cursor-pointer flex items-center justify-center gap-2"
              >
            ...
            {service.provider && (
              <div className="rounded-2xl border border-slate-850 bg-slate-900/40 p-6 backdrop-blur-md space-y-4">
                <p className="text-[10px] uppercase tracking-widest text-slate-500 font-bold">Service Professional</p>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-200 text-sm">{service.provider.name}</h4>
                    <p className="text-[10px] text-slate-500 uppercase tracking-widest">Certified Pro</p>
                  </div>
                </div>
